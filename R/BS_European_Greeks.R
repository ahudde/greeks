...
BS_European_Greeks <-
  ...
  if (payoff == "call") {
    ...
    if ("charm" %in% greek) {
      result["charm"] <-
        ...
        (2 * (r - dividend_yield) * time_to_maturity -
           d2 * volatility * sqrt(time_to_maturity)) /
          (2 * time_to_maturity * volatility * sqrt(time_to_maturity)))
    }
    if ("vomma" %in% greek) {
      result["vomma"] <-
        initial_price * exp(-dividend_yield  * time_to_maturity) * dnorm(d1) *
    ...
    if ("vera" %in% greek) {
      result["vera"] <-
        exercise_price * time_to_maturity * exp(-r * time_to_maturity) *
        dnorm(d2) * -(sqrt(time_to_maturity) + d2 / volatility)
    }
